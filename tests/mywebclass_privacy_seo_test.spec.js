...
test.describe('MyWebClass.org tests', () => {
  test('SEO tests', async ({ page }) => {
    ...
    const pageMetaDescription = await page.$eval(
      'meta[name="description"]',
      (el) => el.content
    );
    const pageMetaKeywords = await page.$eval(
      'meta[name="keywords"]',
      (el) => el.content
    );
    const hasMetaTags = Boolean(
      pageTitle && pageMetaDescription && pageMetaKeywords
    );
    ...
    const hasProperUrlStructure = Boolean(
      pageUrl && !pageUrl.includes('%20') && pageUrlPathname === '/privacy.html'
    );
